...
config_name = "./elbow_insertion_config.yaml" # Configuration file for the runner network.
...
ELBOW_TOOL_OFFSET = RQ85_TCP_OFFSET 
...
# UPDATE WORLD POSE (--> PICK POSE)

# CHANGED KIT - need to fix this
ELBOW_WORLD_POSE = Transform(490.00,827.00,80.00,3.1415900,0.00000,1.570796370) #Transform(-80.77999877929688, 195.26998901367188, 0, 0, 0, 0) #-1.5707963705062866, -1.5707963705062866, 0.0)

ELBOW_GRASP_OFFSET = Transform(0, 0, 10.0, 0.5, -0.5, -0.5, 0.5)

ELBOW_PICK_POSE = ELBOW_WORLD_POSE.multiply(ELBOW_GRASP_OFFSET) # world pose of gripper when picking up elbow, including grasp offset
...
PEG_LOCATION = Transform(80, 675, 180, 0.0, 0.0, -1.047197699546814)
ELBOW_PRE_INSERT = Transform(0,0,10, 0, 0,0).multiply(PEG_LOCATION) # add small z offset to known peg location to ensure we don't start in collision
...
flange_calib_transform = Transform(0, 0, -17, 0, 0, 0, 1)
...
def extract_action(robot, action):
    ...
    default_action = _compute_default_action(robot)
    ...
    print("Checking environment-informed action")
    print(default_action_position)
...
def _compute_default_action(robot):
    ...
    ELBOW_TCP = ELBOW_GRASP_OFFSET.multiply(ELBOW_TOOL_OFFSET.invert()) 
    nominal_goal_pose = PEG_LOCATION.multiply(ELBOW_TCP)
...
if(__name__=="__main__"):
    ...
    # Peg is fixed, hole should be === elbow. elbow is r1, so ...
    # We initialise UR10e-1 just to ensure it's out of the way
    null_robot = workcell.get_robot("UR10e-1") #check these 
    elbow_robot = workcell.get_robot("UR10e-0")
    null_gripper = workcell.get_gripper("Rq85-1")
    elbow_gripper = workcell.get_gripper("Rq85-0")

    # get force sensor(s)
    elbow_f_sensor = workcell.get_force_sensor('UR10e-0')

    # ------- DEBUG --------------
    # Visualize elbow pose:
    workcell.add_part("ONDRIVES_US_CORP_103_16_2222", "elbow1", ELBOW_WORLD_POSE)
    ...
    try:
        if not (elbow_robot.get_flange_transform().is_equal(5, 0.03, WORLD_T_R0_HOME)):
            pyaraas.run(pyaraas.Task(joint_path_execute, workcell, elbow_robot, SAFE_JOINT_0, 5.0))

        if not (null_robot.get_flange_transform().is_equal(5, 0.03, WORLD_T_R1_HOME)):
            pyaraas.run(pyaraas.Task(joint_path_execute, workcell, null_robot, SAFE_JOINT_1, 5.0))
    ...
    adjusted_pose =  flange_calib_transform.multiply(elbow_robot.get_flange_transform())
    print(adjusted_pose)
    ...
    pyaraas.run(pyaraas.Task(elbow_gripper.open, 35,1))

    # pick up elbow:
    elbow_grasp_pose = ELBOW_PICK_POSE.multiply(ELBOW_TOOL_OFFSET.invert())
    elbow_pregrasp_pose = approach.multiply(elbow_grasp_pose)

    try:
        pyaraas.run(pyaraas.Task(araas_path_planner, workcell, elbow_robot, elbow_pregrasp_pose, 3.0))
    except Exception:
    ...
    pyaraas.run(pyaraas.Task(araas_small_move, elbow_robot, elbow_grasp_pose, 1.0))
    pyaraas.run(pyaraas.Task(elbow_gripper.open, 0, 1))
    pyaraas.run(pyaraas.Task(araas_small_move, elbow_robot, elbow_pregrasp_pose, 1.0))
    pickup_buffer = Transform(elbow_pregrasp_pose)
    pickup_buffer.position.z += 200
    pyaraas.run(pyaraas.Task(araas_small_move, elbow_robot, pickup_buffer, 1.0))

    print("Completed elbow pickup")
    ...
    ELBOW_TCP = ELBOW_GRASP_OFFSET.multiply(ELBOW_TOOL_OFFSET.invert()) 
    ELBOW_START_INSERT = ELBOW_PRE_INSERT.multiply(ELBOW_TCP) # starting flange transform 
    
    buffer_pose = Transform(ELBOW_START_INSERT)
    buffer_pose.position.z += 200
    buffer_pose.position.y -= 100
            
    try:
        pyaraas.run(pyaraas.Task(araas_path_planner, workcell, elbow_robot, buffer_pose, 3.0))
        pyaraas.run(pyaraas.Task(araas_small_move, elbow_robot, ELBOW_START_INSERT, 2.0))
    except Exception:
        print(traceback.format_exc())
        import sys
        sys.exit() 
    ...
    try:
        pyaraas.run(pyaraas.Task(insertion_command_distributor, elbow_robot, elbow_f_sensor, max_action_time, socket))
    except Exception:
        traceback.print_exc()
        import sys
        sys.exit()
    ...
    pyaraas.run(pyaraas.Task(araas_small_move, elbow_robot,buffer_pose, 1.0))
