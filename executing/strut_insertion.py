...
# USING SAME RUNNER AND CONFIG AS ELBOW - the process should be generalized enough to transfer
# straight over without training, as long as we include the appropriate target offsets when sending observations to the runner

config_name = "./strut_insertion_config.yaml" # Configuration file for the runner network.
...
SAFE_JOINT_1 = [0.62744587, -2.11185, 2.11185, -1.5707963267948966, -1.5707963267948966, -1.23918]
...
STRUT_WORLD_POSE = Transform(475.00,835.00,20.00,0.00000,0.00000,-1.57080)
...
STRUT_PICK_POSE = STRUT_WORLD_POSE.multiply(STRUT_GRASP_OFFSET) # world pose of gripper when picking up elbow, including grasp offset
...
PEG_LOCATION = Transform(80, 685, 238, 0.0, 0.0, -1.047197699546814) # TOP OF ELBOW}
# start much higher for strut
insert_delta = 15 # buffer value, may not need
STRUT_PRE_INSERT = Transform(0,0,insert_delta, 0, 0,0).multiply(PEG_LOCATION) # add small z offset to known peg location to ensure we don't start in collision
...
# =============================================================================
# If training regime was correctly set up, we shouldn't need to adjust static 
# offset poses of robot, goal points, etc
# ... however if we alter the workcell we do have to take that into account. Add a calibration transform
# to correct for any variations in the workcell vs training environment

# If we use the same policy to insert strut as elbow, we need to add an offset along the strut vertical transform (I think grasp Y?)
# It includes total elbow length + any additional offset between (elbow grasp offset) and (strut grasp offset)
# elbow grasp was 10mm in Z, strut grasp is (currently) 50mm in Z, elbow height is 35 mm, delta = 50-10+35 = 75mm

delta = 15 + STRUT_GRASP_OFFSET.position.z - 10 # Trying to accommodate disparity in grasp height
flange_calib_transform = Transform(0, delta, -17, 0, 0, 0, 1)
...
def extract_action(robot, action, nominal_goal_pose):
    ...
    print("Checking action goal") # weirdly this looks fine in sim - possibly only an issue with the sensor?
    print(goal_pose.get_values())
...
async def insertion_command_distributor(robot, ft, action_time, socket):
    ...
    STRUT_TCP = STRUT_GRASP_OFFSET.multiply(STRUT_TOOL_OFFSET.invert()) 
    nominal_goal_pose = PEG_LOCATION.multiply(STRUT_TCP) # includes elbow offset
    print("nominal goal: adjust strut TCP to change")
    ...
    while not done:
        ...
        # World origin and robot origin in training and execution should be identical, no need to adjust

        # but we flipped the elbow for pickup. Now, this shouldn't affect the insertion state, because we flip back to insert, right?
        # (check that, actually.)
        # the input is the flange pose of the robot, output is action - I think flipping the action should work. But first, make sure the robot turns over?
        # So we either need to flip the observation state to match the training state, or flip the action to match
        # the workcell

        obs_pose = flange_calib_transform.multiply(robot.get_flange_transform())
...
if(__name__=="__main__"):
    ...
    # Use r0 to insert strut, r1 to hold elbow. 
    elbow_robot = workcell.get_robot("UR10e-1") #check these 
    strut_robot = workcell.get_robot("UR10e-0")
    elbow_gripper = workcell.get_gripper("Rq85-1")
    ...
    # ------- DEBUG --------------
    # Visualize elbow pose:
    workcell.add_part("strut", "strut1", STRUT_WORLD_POSE)

    #print(PLATFORM_POSE.multiply(PLATFORM_T_ELBOW))
    ...
    try:
        if not (strut_robot.get_flange_transform().is_equal(5, 0.05, WORLD_T_R0_HOME)):
            pyaraas.run(pyaraas.Task(joint_path_execute, workcell, strut_robot, SAFE_JOINT_0, 5.0))

        if not (elbow_robot.get_flange_transform().is_equal(5, 0.05, WORLD_T_R1_HOME)):
            pyaraas.run(pyaraas.Task(joint_path_execute, workcell, elbow_robot, SAFE_JOINT_1, 5.0))
    ...
    print("Defining new world home poses")
    print(strut_robot.get_flange_transform().get_values())
    print(elbow_robot.get_flange_transform().get_values())
    print(strut_robot.get_joint_angles())
    print(elbow_robot.get_joint_angles())

    #print("Sanity check")
    #print("actual flange position")
    #print(strut_robot.get_flange_transform())
    #print("Training flange equivalent")
    #adjusted_pose =  flange_calib_transform.multiply(strut_robot.get_flange_transform())
    #print(adjusted_pose)
    ...
    elbow_robot.set_collision_model(True, True)
    elbow_gripper.set_collision_model(True, True)
    ...
    # pick up elbow:
    strut_grasp_pose = STRUT_PICK_POSE.multiply(STRUT_TOOL_OFFSET.invert())
    ...
    pickup_buffer = Transform(strut_pregrasp_pose)
    pickup_buffer.position.z += 200
    pyaraas.run(pyaraas.Task(araas_small_move, strut_robot, pickup_buffer, 1.0))

    print("Completed strut pickup")

    print("TO-DO: use R1 here (and eventually, simultaneous with pickup) to grab elbow")

    try:
        pyaraas.run(pyaraas.Task(araas_path_planner, workcell, elbow_robot, R1_ELBOW_PLANNED_POSE, 3.0))
    except Exception:
        print(traceback.format_exc())
        import sys
        sys.exit()
    ...
    pyaraas.run(pyaraas.Task(elbow_gripper.open, 40, 1))
    pyaraas.run(pyaraas.Task(araas_small_move, elbow_robot, R1_ELBOW_PREGRASP, 1.0))
    pyaraas.run(pyaraas.Task(araas_small_move, elbow_robot, R1_ELBOW_GRASP, 1.0))
    pyaraas.run(pyaraas.Task(elbow_gripper.open, 0, 1))
    ...
    STRUT_TCP = STRUT_GRASP_OFFSET.multiply(STRUT_TOOL_OFFSET.invert()) 
    STRUT_START_INSERT = STRUT_PRE_INSERT.multiply(STRUT_TCP) # starting flange transform 
    
    buffer_pose = Transform(STRUT_START_INSERT)
    buffer_pose.position.z += 200
    buffer_pose.position.y -= 100
            
    try:
        pyaraas.run(pyaraas.Task(araas_path_planner, workcell, strut_robot, buffer_pose, 3.0))
        pyaraas.run(pyaraas.Task(araas_small_move, strut_robot, STRUT_START_INSERT, 2.0))
    except Exception:
        print(traceback.format_exc())
        import sys
        sys.exit() 
    ...
    # ----------------------------
    elbow_robot.set_collision_model(False, False)
    elbow_gripper.set_collision_model(False, False)
    strut_robot.set_collision_model(False, False)
    strut_gripper.set_collision_model(False, False)
    ...
    initialise_policy_states(elbow_robot, socket)

    # command distributor waits for query/action input from policy runner, enacts action, responds with new state
    print("Checking current flange orientation")
    print(elbow_robot.get_flange_transform())

    try:
        pyaraas.run(pyaraas.Task(insertion_command_distributor, strut_robot, strut_f_sensor, max_action_time, socket))
    except Exception:
        traceback.print_exc()
        import sys
        sys.exit()
