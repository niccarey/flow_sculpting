...
from rl_components.my_running_mean_std import RunningMeanStd, RunningMeanStdObs
...
class ModelA2CMultiDiscrete(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            # this will never be true?? and yet we still manage to train?
            is_train = input_dict.get('is_train', True)
            action_masks = input_dict.get('action_masks', None)
            prev_actions = input_dict.get('prev_actions', None)
            input_dict['obs'] = self.norm_obs(input_dict['obs'])
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            # ... train is not set on A2C even while training? This feels like a bug
            if (not is_train):
                # need to fix this for isaac run as well
                if type(input_dict['obs']) is dict:
                    if 'policy' in input_dict['obs']:
                        run_device = input_dict['obs']['policy'].device

                else:
                    run_device = input_dict['obs'].device

                mu = mu.detach().cpu().to(run_device)
                logstd = logstd.detach().cpu().to(run_device)
                value = value.detach().cpu().to(run_device)
