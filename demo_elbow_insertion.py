...
ELBOW_WORLD_POSE = Transform(-80.77999877929688, 195.26998901367188, -0.49000048637390137, -1.5707963705062866, -1.5707963705062866, 0.0)
ELBOW_GRASP_OFFSET = Transform(-19.5, 7.690000057220459, -20.0, -1.5707963705062866, 0.0, -3.141592502593994)
...
PEG_LOCATION = Transform(104.02638244628906, 670.6012573242188, 211.86000061035156, 0.0, 0.0, -1.047197699546814)
ELBOW_PRE_INSERT = Transform(0,0,3, 0, 0,0).multiply(PEG_LOCATION) # add small z offset to known peg location to ensure we don't start in collision
...
if(__name__=="__main__"):
    ...
    null_robot = workcell.get_robot("UR10e-0") #check these 
    elbow_robot = workcell.get_robot("UR10e-1")
    null_gripper = workcell.get_gripper("Rq85-0")
    ...
    try:
        if not (elbow_robot.get_flange_transform().is_equal(5, 0.03, WORLD_T_R1_HOME)) and not (null_robot.get_flange_transform().is_equal(0.1, 0.01, WORLD_T_R0_HOME)):
            pyaraas.run([pyaraas.Task(joint_path_execute, workcell, elbow_robot, SAFE_JOINT_1, 5.0), pyaraas.Task(joint_path_execute, workcell, null_robot, SAFE_JOINT_0, 5.0)])

        elif not (elbow_robot.get_flange_transform().is_equal(5, 0.03, WORLD_T_R1_HOME)):
